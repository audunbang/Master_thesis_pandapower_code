...
class load_profiles(object):
    ...
    def calculate_sum_per_timepoint(self):
        """
            Calculate the sum of values for each timepoint (column) in the load profile.
            
            Returns:
                timepoint_sums: A pandas Series containing the sum of values for each timepoint (column) in the load profile.
        """
        timepoint_sums = self.loaddata.iloc[:, 0:].sum(axis=1)
        return timepoint_sums